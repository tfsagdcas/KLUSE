...
#         return spans
class TokenDecoder(BaseDecoder):
    START_END_THRESHOLD = 0.01   # 避免爆炸 & 不杀弱实体
    INSIDE_THRESHOLD = 0.0       # 允许弱 inside，通过长实体

    def get_indices_above_threshold(self, scores, threshold):
        scores = torch.sigmoid(scores)
        return [k.tolist() for k in torch.where(scores > threshold)]

    def calculate_span_score(self, start_idx, end_idx, scores_inside_i,
                             start_i, end_i, id_to_classes):
        span_i = []
        for st, cls_st in zip(*start_idx):
            for ed, cls_ed in zip(*end_idx):
                if ed >= st and cls_st == cls_ed:
                    ...
                    # ★ 关键：不要过滤中文医学实体的 inside token
                    # if (ins < 0.05).any(): continue
    ...
    def decode(self, tokens, id_to_classes, model_output, flat_ner=False,
               threshold=0.1, multi_label=False, **kwargs):
        ...
        for i, _ in enumerate(tokens):
            ...
            # ★ start 和 end 使用轻过滤，避免漏实体也避免爆炸
            start_idx = self.get_indices_above_threshold(scores_start[i], self.START_END_THRESHOLD)
            end_idx   = self.get_indices_above_threshold(scores_end[i], self.START_END_THRESHOLD)
            ...
            # 最终使用 threshold 过滤一下弱噪声实体
            span_i = [s for s in span_scores if s[-1] >= threshold]
            span_i = self.greedy_search(span_i, flat_ner, multi_label)
